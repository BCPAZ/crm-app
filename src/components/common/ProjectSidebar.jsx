      ...
      <div className="p-5 overflow-y-auto">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            {companyData?.image_url && <div className="w-10 h-10 rounded-lg shrink-0 overflow-hidden"><img className="w-full h-full object-cover shrink-0" src={companyData?.image_url} alt={companyData?.name} /></div>}
            <span className="text-md font-medium">{companyData?.name}</span>
          </div>
          <button onClick={() => dispatch(closeProjectSidebar())} className="p-2 rounded-lg h-fit bg-gray-200"><MdClose size={20} color="gray" /></button>
        </div>
